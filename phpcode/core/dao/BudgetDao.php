<?php
  ...
  * permanently store and retrieve Budget object instances. 
  */
...
class BudgetDao {
    ...
    function load(&$conn, &$valueObject) {
          ...
          $sql = "SELECT * FROM budget WHERE (id = ".$valueObject->getId().") "; 
    ...
    function loadAll(&$conn) {
          ...
          $sql = "SELECT * FROM budget ORDER BY id ASC ";
    ...
    function create(&$conn, &$valueObject) {
          ...
          $sql = "INSERT INTO budget ( quantity, period, user) VALUES (".$valueObject->getQuantity().", ";
          $sql = $sql."".$valueObject->getPeriod().", ";
    ...
    function save(&$conn, &$valueObject) {
          ...
          $sql = "UPDATE budget SET quantity = ".$valueObject->getQuantity().", ";
          $sql = $sql."period = ".$valueObject->getPeriod().", ";
          $sql = $sql."user = ".$valueObject->getUser()."";
    ...
    function delete(&$conn, &$valueObject) {
          ...
          $sql = "DELETE FROM budget WHERE (id = ".$valueObject->getId().") ";
          $result = $this->databaseUpdate(&$conn, $sql);
    ...
    function searchMatching(&$conn, &$valueObject) {
          ...
          $first = true;
          $sql = "SELECT * FROM budget WHERE 1=1 ";
          ...
          if ($valueObject->getPeriod() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND period = ".$valueObject->getPeriod()." ";
          }

          if ($valueObject->getUser() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND user = ".$valueObject->getUser()." ";
          }
    ...
    function singleQuery(&$conn, &$sql, &$valueObject) {
          ...
          if ($row = $conn->nextRow($result)) {
                   ...
                   $valueObject->setId($row[0]); 
                   $valueObject->setQuantity($row[1]); 
                   $valueObject->setPeriod($row[2]); 
                   $valueObject->setUser($row[3]); 
          } else {
    ...
    function listQuery(&$conn, &$sql) {
          ...
          while ($row = $conn->nextRow($result)) {
               ...
               $temp->setId($row[0]); 
               $temp->setQuantity($row[1]); 
               $temp->setPeriod($row[2]); 
               $temp->setUser($row[3]); 
