<?php
  ...
  * permanently store and retrieve Expense object instances. 
  */
...
class ExpenseDao {
    ...
    function load(&$conn, &$valueObject) {
          ...
          $sql = "SELECT * FROM expense WHERE (id = ".$valueObject->getId().") "; 
    ...
    function create(&$conn, &$valueObject) {
          ...
          $sql = "INSERT INTO expense ( date, quantity, intake, ";
          $sql = $sql."budget) VALUES ('".$valueObject->getDate()."', ";
          $sql = $sql."".$valueObject->getQuantity().", ";
          $sql = $sql."".$valueObject->getIntake().", ";
          $sql = $sql."".$valueObject->getBudget().") ";
    ...
    function save(&$conn, &$valueObject) {
          ...
          $sql = "UPDATE expense SET date = '".$valueObject->getDate()."', ";
          $sql = $sql."quantity = ".$valueObject->getQuantity().", ";
          $sql = $sql."intake = ".$valueObject->getIntake().", ";
          $sql = $sql."budget = ".$valueObject->getBudget()."";
    ...
    function delete(&$conn, &$valueObject) {
          ...
          $sql = "DELETE FROM expense WHERE (id = ".$valueObject->getId().") ";
          $result = $this->databaseUpdate(&$conn, $sql);
    ...
    function searchMatching(&$conn, &$valueObject) {
          ...
          if ($valueObject->getDate() != "") {
              if ($first) { $first = false; }
              $sql = $sql."AND date = '".$valueObject->getDate()."' ";
          }

          if ($valueObject->getQuantity() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND quantity = ".$valueObject->getQuantity()." ";
          }

          if ($valueObject->getIntake() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND intake = ".$valueObject->getIntake()." ";
          }

          if ($valueObject->getBudget() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND budget = ".$valueObject->getBudget()." ";
          }
    ...
    function singleQuery(&$conn, &$sql, &$valueObject) {
          ...
          if ($row = $conn->nextRow($result)) {
                   ...
                   $valueObject->setId($row[0]); 
                   $valueObject->setDate($row[1]); 
                   $valueObject->setQuantity($row[2]); 
                   $valueObject->setIntake($row[3]); 
                   $valueObject->setBudget($row[4]); 
          } else {
    ...
    function listQuery(&$conn, &$sql) {
          ...
          while ($row = $conn->nextRow($result)) {
               ...
               $temp->setId($row[0]); 
               $temp->setDate($row[1]); 
               $temp->setQuantity($row[2]); 
               $temp->setIntake($row[3]); 
               $temp->setBudget($row[4]); 
